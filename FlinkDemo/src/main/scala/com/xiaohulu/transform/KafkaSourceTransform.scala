...
import com.xiaohulu.bean.{AnchorTrans, GoodsTrans}
import com.xiaohulu.tools.DateUtil
...
object KafkaSourceTransform {
  ...
  def getGoodsIncrInfo(goods: DataStream[GoodsResultBean]): DataStream[(String, String, String, String, Int, Double, String, Int, Long)] = {
    ...
    val resStream = goods.map(a => {
      ...
      var date = a._5
      if (timeStamp < b._4) {
        salesNumber = b._2 //取出窗口间隔内最大时间的销量
        timeStamp = b._4
        date = b._5
      }
      if (startTime > b._4) {
        startTime = b._4 //取出窗口间隔内的最小时间
      }
      var minPrice = a._3
      if (minPrice > b._3) {
        minPrice = b._3 //取最低价 这个是全窗口内的
      }
      incr = salesNumber - befSal
      //befSal = salesNumber
      (a._1, salesNumber, minPrice, timeStamp, date, incr, startTime)
    }).uid("s14")
      .map(x => {
        val timestamp = x._4 * 1000L
        val strDateTime = DateUtil.timestampToDateStr(timestamp)
        val timeMinut = strDateTime.substring(0, 12)
        (x._1._2, x._1._3, timeMinut, x._1._1, x._2, x._3, x._5, x._6, x._7)
      }).uid("s15")
  ...
  def getAnchorInfo(anchor: DataStream[AnchorResultBean]): DataStream[(String, String, String, Int, Int, Long, String)] = {
    val resStreamAnchor = anchor.map(a => {
      ...
      //.assignTimestampsAndWatermarks(new MyPeriodicWatermark)
      .assignTimestampsAndWatermarks(new BoundedOutOfOrdernessTimestampExtractor[AnchorTrans](Time.seconds(5)) {
      ...
      .timeWindow(Time.minutes(10), Time.minutes(1)) //本地
      .reduce((a, b) => {
      var mtotalViewer = a._2 //要求的这一分钟内的最大人气
      var wintotalViewer = a._2 //本滑动步长内的最大时间人气
      var monlineViewer = a._3 //要求的这一分钟内的最大在线人数
      var winonlineViewer = a._3 //本滑动步长内的最大在线人数
      var startTime = a._4
      var timestamp = a._4
      var date = a._5
      if (startTime < b._4) {
        timestamp = b._4
        date = b._5
        wintotalViewer = b._2 //取出窗口间隔内最大时间的人气
        winonlineViewer = b._3 //取出窗口间隔内最大时间的在线人数
      }
      if (mtotalViewer < b._2) {
        mtotalViewer
      }
      if (monlineViewer < b._3) {
        monlineViewer
      }
      (a._1, wintotalViewer, winonlineViewer, timestamp, date, mtotalViewer, monlineViewer)
    }).uid("s19")
      .map(x => {
        val timestamp = x._4 * 1000L
        val strDateTime = DateUtil.timestampToDateStr(timestamp)
        val timeMinut = strDateTime.substring(0, 12)
        (x._1._1, x._1._2, timeMinut, x._6, x._7, x._4, x._5)
      }).uid("s20")
