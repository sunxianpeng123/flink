...
import com.xiaohulu.transform.KafkaSourceTransform
...
object KafkaProcessWindowT2 {
  ...
  def main(args: Array[String]): Unit = {
    ...
    //https://blog.csdn.net/xt1233/article/details/101369515
    //在 Flink Table 的新架构中，有两个查询处理器：Flink Query Processor 和 Blink Query Processor，分别对应两个Planner，我们称之为 Old Planner 和 Blink Planner。
    //Flink 1.11 Table/SQL 的 blink planner 成为默认 Planner
    ...
    val t1 = ConfigTool.anchor_basic_info_topics
    val t2 = ConfigTool.anchor_goods_topics
    topics.+=(t1)
    topics.+=(t2)
    //val kafkaSource = new FlinkKafkaConsumer[String](topics, new SimpleStringSchema(), properties)
    val kafkaSource = new FlinkKafkaConsumer[String](topics, new SimpleStringSchema(), ConfigTool.kafkaProps)
    //指定kafka的消费 位点
    ...
    //商品数据
    tableEnv.createTemporaryView("goods", resStreamGoods, 'room_id, 'live_id, 'time_minut, 'promotion_id, 'sales_number, 'min_price, 'date, 'incr_sales, 'start_time)
    //主播数据
    tableEnv.createTemporaryView("anchor", resStreamAnchor, 'room_id, 'live_id, 'time_minut, 'totalviewer, 'onlineviewer, 'timestamp, 'date)
